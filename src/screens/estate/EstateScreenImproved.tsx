// screens/estate/EstateScreenImproved.tsx
...
const EstateScreenImproved = () => {
  ...
  // Reload family members when modal opens to ensure we have latest data
  useEffect(() => {
    if (isModalVisible) {
      loadFamilyMembers();
    }
  ...
  const handleOpenModal = (plan?: any) => {
    if (plan) {
      ...
      // Sync beneficiary names with current family members
    ...
    try {
      const submitData = {
        ...
        beneficiaries: formData.beneficiaries.map((b) => {
          // Ensure name is properly set by looking up from family members if needed
          const member = familyMembers.find((m: any) => m._id === b.familyMemberId);
  ...
  const getBeneficiaryName = (beneficiary: any) => {
    ...
    // First try to get name from beneficiary object
    if (beneficiary.name && beneficiary.name.trim()) {
      return beneficiary.name;
    }

    // If not available, look up in family members by ID
    ...
    // Try alternative field names that might be in the data
  ...
  return (
    <SafeAreaView style={styles.container} edges={['top']}>
      ...
      {/* Create/Edit Modal - FULLY SCROLLABLE */}
      <Modal
...
export default EstateScreenImproved;
